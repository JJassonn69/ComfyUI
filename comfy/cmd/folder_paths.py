...
def init_default_paths(folder_names_and_paths: FolderNames, configuration: Optional[Configuration] = None, create_all_directories=False, replace_existing=True):
    ...
    # Add fallback for when base_paths is not in configuration
    config_base_paths = getattr(configuration, 'base_paths', [])
    cwd = Path(configuration.cwd) if hasattr(configuration, 'cwd') and configuration.cwd is not None else None
    base_paths = [cwd] if cwd is not None else []
    base_paths.extend(config_base_paths)
